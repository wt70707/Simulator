...
#include <gazebo_block_laser_pcl2/gazebo_ros_block_laser_pcl2.h>
...
void GazeboRosBlockLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->GetElement("pcl"))
  {
   ROS_INFO("Block laser plugin missing <pcl>, defaults to pcl2");
  this->pcl_=2;
  }
  else
    this->pcl_= _sdf->GetElement("pcl")->Get<double>();
